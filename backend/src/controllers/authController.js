...
const resetPassword = async (req, res, next) => {
    try {
        ...
        /**
         * Q1：攻击者拿到数据库里的哈希，能不能直接用来重置密码？
         * 不能。因为重置接口要求用户提供的是原始令牌，而不是哈希。后端会先对原始令牌做哈希，再比较数据库里的哈希。攻击者如果只有哈希，无法生成一个能通过验证的原始令牌。
         *
         * Q2：为什么要随机生成 32 字节？为什么不是 16 字节？
         * 32 字节 = 256 位，生成的原始令牌有 16^64 种可能（约 1.16e77），暴力枚举是不可能的。更短的令牌虽然也够用，但 256 位是行业推荐的安全长度。
         *
         * Q3：哈希之后还需要加盐（salt）吗？
         * 不需要。因为原始令牌本身就是完全随机的、高熵的，不是用户自己设置的弱密码。
         * 加盐主要是为了防止彩虹表攻击，但这里的哈希值攻击者无法直接用来冒充（因为接口需要原始令牌），而且 256 位随机数根本不在彩虹表范围内。所以不加盐也足够安全。
         *
         *
         */
